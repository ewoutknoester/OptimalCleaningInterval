...
# Define costs for tree construction and deployment
...
dive.cost <- 16 # USD: 15 EUR per dive per diver
material.cost <- 17.05 # USD: Cement, PVC, loops etc (see Masterplan Excel), in 2019 USD
hour.wage <- 0.46 # USD: 500 ksh for a 8h day
initial.cost.structure <- material.cost + hour.wage*8 
...
ggplot(P.costs, aes(x = Treatment, y = Costs.fragment, fill = `Cost type`))+
  ...
  labs(y = "Cost per fragment (2019 USD)",
       x = "Cleaning frequency")+
  scale_y_continuous(expand = c(0,0), limits=c(0, 11), breaks = c(0, 2, 4, 6, 8),  labels=scaleFUN)+
  theme_economist()+scale_colour_economist()+
  ...
  geom_text(aes(label = ifelse(is.na(Costs_tot), "", paste0("$",  format(round(Costs_tot, digits = 2), nsmall = 2)))), vjust = -1.2, size = 4, color="black", fontface = "bold")+

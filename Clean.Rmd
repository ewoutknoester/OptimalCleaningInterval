...
sgr.summary.SST <- sgr.summary.SST[order(as.character(sgr.summary.SST$Finder)),]

# Perform Tukey to get significance letters
hsd.SST=HSD.test(aov(SGR~Species*Size*Treatment, data=sgr.data), trt = c("Species", "Size", "Treatment"),
                 group = T)
...
ggplot(P.summary.T, aes(x = Treatment, y = EV.increase))+
  geom_bar(stat = "identity", position = position_dodge())+
  labs(y = expression(paste("Volume increase (% ", y^-1, ")")),
       x = "Cleaning treatment")+
  geom_errorbar(aes(ymin=EV.increase-(1*se), ymax=EV.increase+(1*se)), width=.2, position=position_dodge(.9))+
  geom_text(data = P.summary.T, aes(x=Treatment, y = EV.increase + se*1.15, label = siglet), vjust=0,
...
ggsave("EVincrease_Treatment.png")
...
# Calculate duration for each Treatment to reach 3900% increase
...
P.costs <- P.costs %>% mutate(Cleaning.visits = Days/Interval) # Number of cleaning visits
P.costs$Cleaning.visits[P.costs$Treatment == "Never"] <- 0
P.costs <- P.costs %>% mutate(Trees.cleaned = c(5,4,3, 1)) # Trees cleaned in one visit (less when dirtier)

dive.cost <- 15 # 15 EUR per dive per diver: 1 diver can between 3 - 5 trees, depending on Treatment (how dirty)
initial.cost <- 20 # 20 EUR for construction costs, deployment & filling tree with fragments

P.costs <- P.costs %>% mutate(Costs = Cleaning.visits * (dive.cost/Trees.cleaned) + initial.cost)
...
ggplot(P.costs, aes(x = Treatment, y = Costs.fragment))+
  ...
  geom_text(aes(label = paste("€",  round(Costs.fragment, 2))), vjust = 1.5, size = 5, color="white")

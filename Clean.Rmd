...
## Ispect: Indicating that missing fragments are roughly equally distributed across Treatments
...
# Match EV.increase as multiplication factor (as used in costs calculations): i.e. 100% is no change (x1)
P.costs$EV.increase <- P.costs$EV.increase + 100
...
P.costs <- P.costs %>% mutate(Days = log(1600/100)/(log((EV.increase)/100)/365))
...
# Specify number of trees cleaned in one visit by a buddy pair (i.e. less trees are cleaned when they are dirtier)
P.costs <- P.costs %>% plyr::mutate(Trees.cleaned = c(5, 4, 3, 1)) 
...
initial.cost.structure <- material.cost + hour.wage*8 # Divide by 5, assuming a structure will last 5 cycles
initial.costs.deployment <- (dive.cost*2 + hour.wage*2*2)/3 # Divided by 3, as values were for 3 trees

## Calculate cleaning costs based on cleaning visits, dive costs and wages
...
## Isolate the costs needed for each nursery round
...
ggplot(P.costs, aes(x = Treatment, y = Costs.fragment, fill = `Cost type`))+
  ...
  geom_text(aes(label = ifelse(is.na(Costs_tot), "", paste0("$",  format(round(Costs_tot, digits = 2), nsmall = 2)))), vjust = -1.2, size = 5, color="black", fontface = "bold")

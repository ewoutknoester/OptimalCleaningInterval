...
## Fill 'Size' with conditional values from 'Length': i.e. make start length a categorical variable (small, medium, large)
...
# (There must be a neater way to do this (e.g. in the long-format data) but I couldn't figure out how)
...
## Plot to get an idea of the data
...
lme.sgr.1  <- lme(SGR ~ Treatment*Species*Size, method = "ML", na.action = na.omit, random = ~1 | Structure, 
                  data = sgr.data, weights = varIdent(form = ~1 | Species * Size))

### Re-checking for heterogeneity
coplot(resid(lme.sgr.1, type = "normalized") ~ Size | Species, data = sgr.data) # Homogeneity improved

## Model output
anova(lme.sgr.1) # Three-way interaction not significant, but all three 2-way interactions are

```
## Model validation
```{r SGR model validation}

mod <- lme.sgr.1 # set model to be validated
...
hsd.SS <- emmeans(lme.sgr.1, specs = pairwise ~ Size|Species, adjust = "tukey")
...
# Perform post hoc and get significance letters (Compact Letter Display) between Sizes within each Species
hsd.TS <- emmeans(lme.sgr.1, specs = pairwise ~ Treatment|Species, adjust = "tukey")
...
# Plot Species*Size*Treatment bar graph + error bars + letters
ggplot(sgr.summary.TS, aes(x = Species, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge())+ 
  labs(y = expression(paste("SGR (", d^-1,")")), x = "Species")+
  geom_errorbar(aes(ymin=SGR-(1*se), ymax=SGR+(1*se)), width=.2, position=position_dodge(.9))+
  geom_text(data = sgr.summary.TS, aes(x=Species, y = SGR + se*2, label = siglet),
            vjust=0, position = position_dodge(.9))+
...
scale_x_discrete(labels= c("A. formosa", "A. verweyi", "P. verrucosa", "P. cylindrica"))
ggsave("SGR_Treatment x Species.png", width = 23, height = 10, units = "cm")
...
# Perform post hoc and get significance letters (Compact Letter Display) between Sizes within each Species
hsd.TSi <- emmeans(lme.sgr.1, specs = pairwise ~ Treatment|Size, adjust = "tukey")
...
# Plot Species*Size*Treatment bar graph + error bars + letters
ggplot(sgr.summary.TSi, aes(x = Size, fill = Treatment, y = SGR))+
  geom_bar(stat = "identity", position = position_dodge())+ 
...
ggsave("SGR_Treatment x Size.png", width = 23, height = 10, units = "cm")
...
hsd.T <- emmeans(lme.sgr.1, specs = pairwise ~ Treatment, adjust = "tukey")
...
### Model 3 (variable precision for Species * Treatment) preferred based on normality residuals (but see different AIC:)
AIC(bm1, bm2, bm3, bm4)
...
### Model 3 without Bias correction preferred
AIC(bm3, bm3bc, bm3br)

## Model selection fixed part
car::Anova(bm3) # Three way interaction significant

```
## Model validation
```{r AVG survival: model validation}

mod <- bm3 # set model to be validated
op <- par(mfrow = c(2, 4), mar = c(5, 4, 1, 2)) # set layout of figure frame
plot(resid(mod, type = "pearson") ~ fitted(mod)) # fitted vs residuals
abline(0,0)
hist(resid(mod, type = "pearson"), main = "") # histogram residuals
...
# Post hoc comparison
hsd.surv.SST <- emmeans(bm3, specs = pairwise ~ Treatment | Species * Size, adjust = "tukey") 
...
# Labels facet grip
spec.labs <- c("A. formosa", "A. verweyi", "P. verrucosa", "P. cylindrica")
names(spec.labs) <- c("Acropora cf formosa", "Acropora verweyi", "Pocillopora verrucosa", "Porites cylindrica")

# Plot Species*Size*Treatment bar graph + error bars + letters
ggplot(surv.summary.SST, aes(x = Size, fill = Treatment, y = Condition))+
  geom_bar(stat = "identity", position = position_dodge())+ 
  labs(y = "Survival (%)", x = "Size")+ facet_grid(rows=vars(Species), labeller = labeller(Species = spec.labs)) +
  geom_errorbar(aes(ymin=Condition-(1*se), ymax=Condition+(1*se)), width=.2,
                position=position_dodge(.9))+
  geom_text(data = surv.summary.SST, aes(x=Size, y = Condition + se*1.5, label = siglet), vjust=0,
                position=position_dodge(.9))+
  scale_fill_manual(values=c("#A9A9A9", "#7A7A7A", "#5A5A5A", "#3A3A3A"))+
  theme_economist()+scale_colour_economist()+
  theme(
...
ggsave("Survival(AVG)_Species x Size x Treatment.png", width = 23, height = 10, units = "cm")
...
# Post hoc comparison
hsd.surv.TSp <- emmeans(bm3, specs = pairwise ~ Treatment|Species, adjust = "tukey") 
...
# Post hoc comparison
hsd.surv.TSi <- emmeans(bm3, specs = pairwise ~ Treatment|Size, adjust = "tukey") 
...
# Post hoc comparison
hsd.surv.T <- emmeans(bm3, specs = pairwise ~ Treatment, adjust = "tukey") 
...
## LOG transformation of response variable to improve residuals
...
lm.p.5  <- lme(log10(EV.increase) ~ Species*Size*Treatment, random = ~1 | Structure, method = "ML",
                  data = P.data)
...
# Model selection
anova(lm.p.7) # Three-way interaction not significant, but Species*Size is
...
# Perform post hoc and get significance letters (Compact Letter Display) between Sizes within each Species
hsd.P.SS <- emmeans(lm.p.7, specs = pairwise ~ Size|Species, adjust = "tukey")
...
ggplot(P.summary.SS, aes(x = Species, fill = Size, y = EV.increase))+
  geom_bar(stat = "identity", position = position_dodge())+ 
  ...
  geom_text(data = P.summary.SS, aes(x=Species, y = EV.increase + se*2, label = siglet),
            vjust=0, position = position_dodge(.9))+
  scale_fill_manual(values=c("#A9A9A9", "#7A7A7A", "#5A5A5A"))+
  theme_economist()+scale_colour_economist()+
  theme(
...
scale_x_discrete(labels= c("A. formosa", "A. verweyi", "P. verrucosa", "P. cylindrica"))
ggsave("Production(LOG)_Species x Size.png", width = 23, height = 10, units = "cm")
...
# Post hoc comparison
hsd.P.T <- emmeans(lm.p.7, specs = pairwise ~ Treatment, adjust = "tukey", type = "response") 
...
P.costs <- P.costs[order(as.character(P.costs$Treatment)),]
...
ggsave("Cost per fragment.png", width = 23, height = 10, units = "cm")
